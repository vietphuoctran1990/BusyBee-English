  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-center justify-center bg-indigo-950/98 backdrop-blur-xl animate-fade-in" onClick={onClose}>
        <div className="bg-white w-full h-full md:h-[85vh] md:max-h-[760px] md:max-w-5xl md:rounded-[2rem] shadow-[0_0_120px_rgba(0,0,0,0.6)] overflow-hidden flex flex-col relative animate-scale-up" onClick={e => e.stopPropagation()}>
            ...
            {/* Header — with safe-area top padding on full-screen mobile */}
            <div className="px-4 py-3 md:px-5 md:py-3 pt-safe-or-3 flex justify-between items-center border-b border-gray-100 bg-white z-30 shrink-0" style={{ paddingTop: 'max(12px, env(safe-area-inset-top))' }}>
                <div className="flex items-center gap-2 min-w-0">
                    <div className="bg-indigo-600 p-1.5 rounded-lg shrink-0">
                      <BookOpenIcon className="w-4 h-4 text-white" />
                    </div>
                    <div className="min-w-0">
                        <h2 className="text-sm md:text-base font-black text-indigo-900 truncate">{data.title}</h2>
                    </div>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                    {!hasSaved && (
                      <button onClick={handleSave} className="flex items-center gap-1.5 px-3 py-2 bg-pink-500 text-white rounded-xl font-black text-xs shadow-sm hover:bg-pink-600 transition-all active:scale-95 min-h-[36px]">
            ...
            <div className="flex-1 flex flex-col md:flex-row overflow-hidden relative">
                ...
                {/* Trang trái: Hình ảnh */}
                <div className="w-full md:w-1/2 h-[42vw] max-h-[260px] sm:h-[45vw] sm:max-h-[320px] md:h-full md:max-h-none bg-indigo-50 relative flex items-center justify-center overflow-hidden border-b md:border-b-0 md:border-r border-gray-100 shrink-0 md:shrink">
                    ...
                    {/* Page Indicator — min 12px font */}
                    <div className="absolute top-2.5 left-2.5 bg-black/50 backdrop-blur-md text-white px-2.5 py-1 rounded-full font-black text-[11px] md:text-xs border border-white/20 z-20">
                    ...
                    <div className="flex-1 overflow-y-auto no-scrollbar p-3 md:p-5 flex flex-col">
                        <div className="max-w-xl mx-auto w-full space-y-3 md:space-y-4 my-auto">
                            <div className="flex justify-between items-center">
                                <span className="px-2 py-1 bg-indigo-50 text-indigo-500 rounded-md text-[10px] md:text-xs font-black uppercase tracking-widest border border-indigo-100">Lời truyện</span>
                                <button
                                    onClick={() => setShowTranslation(!showTranslation)}
                                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl transition-all font-black text-[10px] md:text-xs border-2 min-h-[36px] ${showTranslation ? 'bg-orange-500 text-white border-orange-400' : 'bg-white text-orange-500 border-orange-100 hover:bg-orange-50'}`}
                                >
                                    <LanguageIcon className="w-3.5 h-3.5" /> {showTranslation ? 'Ẩn dịch' : 'Dịch'}
                                </button>
                            </div>
                    ...
                    <div className="p-3 md:p-5 md:pt-0 bg-white shrink-0">
                        <div className="max-w-xl mx-auto space-y-2 md:space-y-2.5">
                            <button
                                ...
                                className={`w-full py-3 md:py-3.5 ${isPlaying ? 'bg-red-500' : 'bg-indigo-600'} text-white font-black rounded-xl shadow-sm flex items-center justify-center gap-2 transition-all active:scale-95 disabled:opacity-50 group min-h-[48px]`}
                            >
                                {isAudioLoading ? (
                            ...
                            <div className="flex gap-2">
                                <button
                                    onClick={handlePrev}
                                    className="flex-1 py-2.5 md:py-3 bg-gray-50 hover:bg-gray-100 text-gray-500 rounded-xl font-black text-xs md:text-sm flex items-center justify-center gap-2 transition-all active:scale-95 min-h-[44px]"
                                >
                                    <ArrowLeftIcon className="w-3.5 h-3.5" /> Trước
                                </button>
                                <button
                                    onClick={handleNext}
                                    className="flex-1 py-2.5 md:py-3 bg-indigo-50 hover:bg-indigo-100 text-indigo-600 rounded-xl font-black text-xs md:text-sm flex items-center justify-center gap-2 transition-all active:scale-95 border border-indigo-100 min-h-[44px]"
                                >
                                    Sau <ArrowRightIcon className="w-3.5 h-3.5" />
                                </button>
                            </div>
                        </div>
                    </div>
