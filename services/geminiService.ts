...
// Lazy singleton — @google/genai (~287 KB) is only fetched on the first AI call.
...
export const generateCardDetails = async (text: string, accent: AccentType = 'US') => {
  return withRetry(async () => {
    const [ai, { Type }] = await Promise.all([getSafeClient(), loadGenAI()]);
...
export const generateIllustration = async (text: string, refImage?: string): Promise<string | undefined> => {
  return withRetry(async () => {
    const ai = await getSafeClient();
    const parts: any[] = [];
    if (refImage) {
      parts.push({ inlineData: { data: refImage, mimeType: 'image/png' } });
    }
    parts.push({ text: `${CHILD_FRIENDLY_STYLE} Subject: ${text}` });
    const response = await ai.models.generateContent({
...
export const generatePronunciation = async (text: string, _accent: AccentType = 'US'): Promise<string | undefined> => {
  try {
    return await withRetry(async () => {
      const [ai, { Modality }] = await Promise.all([getSafeClient(), loadGenAI()]);
      const response = await ai.models.generateContent({
